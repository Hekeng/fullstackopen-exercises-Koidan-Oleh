sequenceDiagram
    ...
    activate server
    server-->>browser: [{ "content": "HTML is easy", "date": "2026-06-08" }, ... ]
    deactivate server
    ...
    Note right of browser: The browser executes the JavaScript code that fetches the updated JSON

    browser->>server: GET https://studies.cs.helsinki.fi/exampleapp/data.json
    activate server
    server-->>browser: [{ "content": "HTML is easy", "date": "2026-06-08" }, ..., { "content": "My first SPA note", "date": "2026-06-08" }]
    deactivate server
