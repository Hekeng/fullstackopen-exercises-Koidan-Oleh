...
const List = ({ countries }) => {
  if (countries.length > 10) {
    ...
    return filtretList.map((item) => <div key={item.name.common}>{item.name.common}</div>)
  }
  //   else if (filtretList.length === 1) {
  //     result = filtretList[0]
  //   } else {
  //     result = ['No matches found, please try again']
  //   }
...
const APP = () => {
  ...
  useEffect(() => {
    ...
    // console.log('Countries downloadet')
  }, [])
  ...
  const serchCountry = () => {
    ...
    console.log('filtretList : ', filtretList)

    // setDisplayedCountries(filtretList)
    // let result

    // if (filtretList.length > 10) {
    //   result = ['Too many matches, specify another filter']
    // } else if (filtretList.length <= 10 && filtretList.length >= 1) {
    //   result = filtretList.map((item) => item.name.common)
    // } else if (filtretList.length === 1) {
    //   result = filtretList[0]
    // } else {
    //   result = ['No matches found, please try again']
    ...
    return filtretList
    // setDisplayedCountries(displayedList)
  }

  // const perem = serchCountry()

  return (
    <>
      find countries <input value={search} onChange={handleSetSearch} />
      <List countries={serchCountry()} />
      <Notification />
    </>
  )
}
